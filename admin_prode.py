    ...
    if "SEMIS" in reales:
        for eq in u_semis:
            if eq in reales["SEMIS"]: 
                ...
                if eq != reales.get("CAMPEON") and eq != reales.get("SUBCAMPEON") and reales.get("CAMPEON") != "-":
...
st.caption("Utilice estos controles para ingresar los resultados reales del Mundial.")
    ...
    with cols_pantalla[idx_col % 2]: 
        with st.expander(f"**RESULTADOS REALES:** {nombre_grupo}", expanded=False):
            ...
            for i, (idx_L, idx_V) in enumerate(FIXTURE_INDICES):
                ...
                with col_btn:
                    res = st.radio(label="", options=["-", "L", "E", "V"], horizontal=True, key=radio_key)
                with col_res:
                    st.caption(f"{local} vs {visita}")
            ...
            pts1 = st.number_input("Puntos REALES del 1º", 0, 9, key=f"Real_{codigo}_pts1")
            p2 = st.selectbox("🥈 2º REAL", ["-"]+equipos, key=f"Real_{codigo}_2", index=0)
            pts2 = st.number_input("Puntos REALES del 2º", 0, 9, key=f"Real_{codigo}_pts2")
            p3 = st.selectbox("🥉 3º REAL", ["-"]+equipos, key=f"Real_{codigo}_3", index=0)
            pts3 = st.number_input("Puntos REALES del 3º", 0, 9, key=f"Real_{codigo}_pts3")
...
octavos_reales = st.multiselect("🏆 Octavos (16)", TODOS_LOS_EQUIPOS, max_selections=16)
cuartos_reales = st.multiselect("🏆 Cuartos (8)", octavos_reales if len(octavos_reales)==16 else TODOS_LOS_EQUIPOS, max_selections=8)
semis_reales = st.multiselect("🏆 Semis (4)", cuartos_reales if len(cuartos_reales)==8 else TODOS_LOS_EQUIPOS, max_selections=4)
...
with col_cam: campeon_real = st.selectbox("🥇 CAMPEÓN", ["-"]+opc_podio, key="Real_Campeon")
with col_sub: subcampeon_real = st.selectbox("🥈 SUBCAMPEÓN", ["-"]+opc_podio, key="Real_Sub")
with col_ter: tercero_ganador_real = st.selectbox("🥉 3ER PUESTO (Ganador)", ["-"]+opc_podio, key="Real_3ro_Ganador")
...
# ==========================================
# 4. GUARDADO EN LA NUBE Y CÁLCULO
# ==========================================
def obtener_datos():
    ...
    try:
        ...
        st.error(f"❌ ERROR Conexión DB Participantes: {e}")
        return None

def guardar_resultados_en_nube(resultados_dict):
    try:
        datos_json = json.dumps(resultados_dict)
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        contenido_json_texto = st.secrets["google_json"]["contenido_archivo"]
        creds_dict = json.loads(contenido_json_texto, strict=False)
        creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
        ...
        # Abrir la hoja específica 'Resultados_Admin'
        try:
            sheet = client.open(NOMBRE_HOJA_GOOGLE).worksheet("Resultados_Admin")
        except gspread.exceptions.WorksheetNotFound:
            st.error("❌ No encuentro la hoja 'Resultados_Admin'. Créala en Google Sheets.")
            return False
            
        sheet.update_acell('A1', datos_json)
        return True
    except Exception as e:
...
with col_calc:
    if st.button("🔄 PREVISUALIZAR CÁLCULO (Solo Local)"):
        # Validaciones básicas
        partidos_cargados = [v for k, v in RESULTADOS_REALES_DINAMICO["PARTIDOS"].items() if v != "-"]
        grupos_cargados = [v for g in RESULTADOS_REALES_DINAMICO["GRUPOS"].values() for k, v in g.items() if (k in ["1","2","3"]) and v != "-"]
        if not partidos_cargados and not grupos_cargados and not RESULTADOS_REALES_DINAMICO["OCTAVOS"]:
             st.warning("⚠️ No ha cargado ningún resultado real.")
        ...
        with st.spinner("Calculando..."):
            ...
            if datos_usuarios:
                ...
                st.success("✅ Cálculo completado (Previsualización).")
                st.dataframe(df, use_container_width=True)

with col_save:
    if st.button("💾 GUARDAR OFICIALMENTE (Para Scoreboard)"):
        with st.spinner("Guardando en Google Sheets..."):
            if guardar_resultados_en_nube(RESULTADOS_REALES_DINAMICO):
                st.success("✅ ¡Guardado! El Scoreboard ahora verá estos resultados.")
                st.balloons()